...
\begin{document}
	...
	While regulatory frameworks have increasingly shifted toward Expected Shortfall, VaR remains the standard directly backtestable quantile risk measure; conformal extensions to ES are discussed in Section~\ref{sec:discuss} and Appendix~\ref{app:es}.
	...
	This result reflects the 250-day rolling window---consistent with practical implementations under regulatory conventions---and associated estimation noise in parametric tail models; under longer estimation samples or carefully tuned thresholds, EVT-based methods may perform differently.
	Adaptive Conformal Inference \citep{gibbs2021adaptive} yields comparable coverage but typically wider VaR intervals (Table~\ref{tab:baselines}), consistent with its symmetric adjustment mechanism.
	...
	The framework does not recover structural predictive content where none exists; in extreme miscalibration regimes, it provides a statistically valid empirical fallback rather than a model improvement.
