...
\begin{document}
	...
	\begin{frontmatter}
		...
		\begin{abstract}
			...
			foundation-model-based---exhibit systematic miscalibration that can be diagnosed and corrected using a simple scalar functional. This paper
			...
			retraining. An approximate asymptotic coverage result is established for the static split-conformal construction under
			$\beta$-mixing dependence, providing asymptotic support for calibration under weak dependence, without implying finite-sample guarantees. The scalar conformal threshold
			$\hat{q}_V$ serves both as the corrective shift restoring empirical coverage
			...
			2000--2026. Conformal recalibration restores nominal coverage in finite samples across a wide range of models and assets, subject to estimation error and dependence effects, achieving high levels of Basel Green Zone compliance with the rolling implementation approaching full compliance in the evaluated sample, revealing an empirical calibration--efficiency trade-off.
			The results provide strong empirical evidence that forecast validity can be treated as separable from forecast generation: a simple, distribution-free post-processing layer can empirically restore tail coverage without modifying the underlying predictive model.
	...
	This paper addresses a central problem in probabilistic forecasting: how to transform arbitrary predictive distributions into calibrated forecasts under temporal dependence. We adopt a post-processing perspective, treating calibration as a distinct layer applied to arbitrary forecast outputs. This separation is particularly relevant in financial risk management, where regulatory evaluation depends on coverage rather than predictive accuracy.
	More broadly, this perspective aligns with the forecast calibration literature, where the objective is not to improve predictive models per se, but to ensure that their probabilistic outputs satisfy statistical validity criteria. We propose a
	one-sided conformal correction that operates as a model-agnostic
	post-processing layer---requiring neither retraining nor
	distributional assumptions---and targets calibration as a distinct layer from predictive accuracy, in line with the forecast post-processing paradigm. In this sense, it belongs to the forecast
	...
	The contribution rests on a specific claim: under sparse tail data and temporal dependence, a one-parameter post-processing rule is empirically close to the right complexity class for quantile recalibration, and its correction term has independent diagnostic value. First, we introduce the one-sided
	...
	signed miscalibration diagnostic with substantial explanatory power for cross-sectional variation in forecast quality---and document the main limitations, including
	sensitivity to calibration window size, persistence, and
	structural breaks.
	This reframes conformal correction not only as a calibration tool, but as a diagnostic lens for model misspecification.

	While regulatory frameworks have increasingly shifted toward Expected Shortfall, VaR remains the standard directly backtestable quantile risk measure; conformal extensions to ES are discussed in Section~\ref{sec:discuss} and Appendix~\ref{app:es}. Accordingly, Expected Shortfall evaluation is included as a complementary diagnostic rather than a primary calibration target.
	The contribution of the paper is therefore not to improve the underlying predictive model, but to address a core forecasting problem: how to transform arbitrary probabilistic forecasts into statistically valid tail quantile forecasts under temporal dependence. In doing so, we separate forecast generation from forecast calibration, providing a model-agnostic correction that applies uniformly across heterogeneous forecasting systems. The static construction provides asymptotic support for coverage validity; the rolling variant is supported by empirical evidence across the datasets considered.
	The contribution should be interpreted as a framework for forecast calibration rather than a method for improving predictive models. The objective is to restore statistical validity of forecast outputs, not to enhance the underlying predictive structure.
	...
	The contribution lies not in the functional form of the correction---which is deliberately minimal---but in the unified mechanism that detects, quantifies, and corrects miscalibration across heterogeneous forecasting models.
	...
	The result applies only to the static calibration scheme. The rolling procedure introduced in Section~\ref{sec:rolling_method} is an empirical deployment strategy and does not inherit the same formal guarantee; it should therefore be interpreted as an empirically validated deployment strategy rather than a theoretically guaranteed procedure. Accordingly, all performance claims for the rolling variant should be interpreted as empirical rather than theoretical.
	The result is not intended as a sharp finite-sample guarantee. In practical sample sizes, the bound is conservative and primarily serves as theoretical support for the direction of the correction rather than as an operational guarantee.
	The approximation error $\Delta_n$ vanishes as the
	calibration sample increases, consistent with asymptotic coverage behavior.
	...
	In practice, the rolling variant constitutes the operational implementation of the framework, while the static construction provides theoretical anchoring. A local coverage result for the rolling variant, applying Theorem~\ref{thm:dependent_conformal_var} window-by-window under local stationarity, is given in Appendix~\ref{app:rolling_coverage}.
	...
	This distinction is central to the interpretation of the method and to its role as a diagnostic framework rather than a purely corrective procedure. When the conformal adjustment is small relative to the base forecast, the method acts as a recalibration layer that preserves predictive structure. When the adjustment dominates, the procedure reduces to a statistically valid empirical fallback, indicating the absence of usable predictive signal in the underlying model.
	Accordingly, the framework should be interpreted as enforcing valid coverage rather than preserving or recovering predictive structure in severely misspecified models.
	...
	In these cases, the conformal layer does not preserve model-specific predictive structure but instead defaults to a statistically valid empirical quantile. This is not a limitation of the method per se but a direct consequence of enforcing coverage under model misspecification: when predictive structure is absent, any valid calibration procedure must revert to an empirical estimate.
	The framework therefore distinguishes explicitly between models with genuine predictive content and those operating effectively as empirical quantile estimators.
	...
	The theoretical guarantee established in Section~\ref{sec:validity} applies strictly to the static split-conformal construction. The rolling implementation used in practice is an empirical deployment strategy whose strong empirical performance is documented in Section~\ref{sec:rolling_results} but is not supported by an equivalent formal guarantee. No claim is made that the rolling procedure satisfies the same asymptotic guarantees as the static construction.

	The conformal framework empirically corrects heterogeneous miscalibration
	...
	dynamics of the predictive distribution; the framework targets calibration rather than structural model improvement. Despite the substantial improvements in conditional coverage documented in Section~\ref{sec:rolling_results}, residual failures remain concentrated in specific assets. The rolling variant
	...
	Conformal correction achieves the lowest QS (5.24 $\times 10^{-4}$) and highest Green rate (86.6\%), followed by quantile regression on residuals (5.31, 82.4\%). More flexible approaches perform strictly worse, reflecting a fundamental sample-size constraint: at $\alpha = 0.01$ with approximately 1{,}500 test observations per asset, the expected number of tail violations is ${\sim}15$, so any method with $k$ free parameters must estimate its additional degrees of freedom from $O(\alpha T)$ effective tail observations, leading to high variance that offsets any bias reduction. The single-parameter conformal shift and quantile regression on residuals---which reduces to a near-scalar correction at extreme quantiles---cluster at the top, consistent with the one-parameter rule being empirically near-optimal among low-parameter corrections in finite samples at extreme quantiles.

	The comparison extends to purpose-built VaR models. GARCH-N with conformal correction (Table~\ref{tab:master}, Panel~A: QS $= 4.7 \times 10^{-4}$, 21/24 Green) dominates both EVT-POT (QS $= 5.5$, 18/24 Green) and filtered historical simulation (QS $= 5.6$, 13/24 Green) under the short 250-day rolling windows typical of regulatory implementations, despite requiring no tail distribution fitting, no threshold selection, and no GPD estimation. Under longer estimation samples or carefully tuned thresholds, EVT-based methods may perform differently; the result should therefore be read as evidence of robustness under short-sample conditions rather than general dominance over parametric tail models. Adaptive Conformal Inference \citep{gibbs2021adaptive} achieves a higher Green rate than the proposed method, while the proposed method achieves a lower Quantile Score and narrower VaR intervals (Table~\ref{tab:baselines}). This reflects a genuine calibration--sharpness trade-off: ACI's dynamic $\alpha_t$ adjustment prioritises coverage at the cost of wider bands, whereas the fixed-$\alpha$ conformal shift prioritises efficiency. Neither approach dominates; the choice depends on whether the practitioner values tighter capital reserves or more robust coverage.
	...
	This paper develops a one-sided conformal recalibration framework for Value-at-Risk that is supported by an approximate asymptotic coverage result under $\beta$-mixing dependence. Across five time series foundation models, four parametric benchmarks, and 24 assets over 2000--2026, conformal correction achieves Basel Green Zone compliance for a majority of model--asset combinations, with no Red Zone classifications.

	The conformal threshold $\qV$ serves a dual role: as a model-agnostic corrective shift that empirically restores coverage in finite samples, and as a continuous, signed diagnostic that ranks forecasters by tail adequacy and tracks regime-dependent miscalibration. The framework targets validity, not predictive structure: in severely misspecified settings, it provides a statistically valid empirical fallback rather than recovering structural predictive content.

	Several directions remain open: extending distribution-free calibration to Expected Shortfall, developing formal guarantees for rolling implementations under dependence, and integrating conformal recalibration with fine-tuned foundation models. More broadly, the framework should be interpreted as a contribution to forecast calibration under dependence, providing a minimal and interpretable adjustment that enforces statistical validity at the forecast level without altering the underlying predictive mechanism.
