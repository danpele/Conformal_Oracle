...
\begin{document}
	...
	Reliable probabilistic forecasting requires not only sharp predictions but also correct calibration: predicted quantiles must be statistically consistent with realised outcomes. In financial risk management, this requirement is particularly stringent. Value-at-Risk (VaR) forecasts are subject to formal regulatory backtesting \citep{basel1996}, and systematic deviations from nominal coverage translate directly into capital penalties. Yet both classical econometric models and modern machine learning approaches routinely produce miscalibrated tail forecasts, especially under temporal dependence and regime shifts \citep{christoffersen1998, nolde2017elicitability, goel2024tsfmvar, pele2025llmvar}.

	This paper proposes a one-sided conformal correction that operates as a model-agnostic post-processing layer---requiring neither retraining nor distributional assumptions---and targets calibration rather than predictive accuracy. The contribution rests on a specific claim: under sparse tail data and temporal dependence, a one-parameter post-processing rule is empirically close to the right complexity class for quantile recalibration, and its correction term has independent diagnostic value. In this sense, the work belongs to the forecast post-processing tradition of \citet{gneiting2013combining}, \citet{brehmer2021properification}, and \citet{taillardat2016calibrated}, while focusing specifically on tail risk measures under dependence. While regulatory frameworks have increasingly shifted toward Expected Shortfall, VaR remains the only tail risk measure with a well-defined elicitability structure supporting direct backtesting \citep{gneiting2011making}; conformal extensions to ES are discussed in Section~\ref{sec:discuss} and Appendix~\ref{app:es}.

	The contribution is threefold. First, we introduce the one-sided conformal recalibration procedure and establish an approximate coverage result under $\beta$-mixing conditions, providing asymptotic support without relying on i.i.d.\ assumptions. Second, we conduct a large-scale empirical evaluation across five time series foundation models (TSFMs), four parametric benchmarks, and 24 financial assets over 2000--2026. This study provides the first systematic calibration audit of time series foundation models for financial tail risk. Third, we show that the scalar conformal threshold $\qV$ (the empirical quantile of calibration-set forecast errors; see Definition~\ref{def:var_correction}) doubles as a continuous, signed miscalibration diagnostic with substantial explanatory power for cross-sectional variation in forecast quality---and document the main limitations, including sensitivity to calibration window size, persistence, and structural breaks.
	...
	\begin{remark}[Low-parameter optimality under tail sparsity]
		\label{rem:tail_sparsity}
		At significance level $\alpha$ with $T$ test observations, the expected number of tail violations is $\alpha T$. Any recalibration method with $k$ free parameters estimated from tail observations faces an effective sample size of $O(\alpha T)$, so the estimation variance of each parameter scales as $O(k / (\alpha T))$. For $\alpha = 0.01$ and $T = 1{,}500$ (a typical asset in our sample), the effective tail sample is ${\sim}15$ observations. In this regime, a one-parameter correction ($k = 1$) minimises estimation variance while retaining sufficient flexibility to restore coverage. Multi-parameter alternatives (e.g., isotonic regression, scale correction with state-dependent scaling) must partition these ${\sim}15$ tail events across additional degrees of freedom, leading to variance inflation that empirically offsets any bias reduction (Table~\ref{tab:baselines}). This bias--variance trade-off is the statistical rationale for the deliberately minimal design of the conformal shift.
	\end{remark}
	...
	In practice, the rolling variant constitutes the operational implementation of the framework, while the static construction provides theoretical anchoring.
	...
	\begin{remark}[Local coverage of the rolling variant]
		...
		\]
		The local stationarity condition permits regime changes \emph{between} windows, requiring only that the score distribution is approximately stationary within each 250-day block. For $w = 250$, the bound gives $\Delta_{250} \approx 0.15$ (i.e., $\geq$84\% guaranteed coverage per window). This is conservative by construction: empirical rolling coverage exceeds 99\% across the datasets considered (Table~\ref{tab:rolling_vs_static}). The bound formalizes the direction of the correction rather than providing a tight finite-sample guarantee; the full proof is in Appendix~\ref{app:rolling_coverage}.
	\end{remark}
