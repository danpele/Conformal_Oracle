...
\begin{document}
	...
	Table~\ref{tab:baselines} reveals a clear calibration--efficiency trade-off across recalibration methods: Adaptive Conformal Inference \citep{gibbs2021adaptive} achieves the highest proportion of Basel Green outcomes (superior unconditional coverage), while the proposed one-parameter correction delivers the lowest Quantile Score and the sharpest VaR intervals. The methods therefore lie on a calibration--efficiency frontier: procedures that enforce stricter coverage produce wider, less informative intervals, while those that prioritise sharpness may accept marginally lower coverage. The proposed method occupies a favourable position on this frontier, balancing reliable coverage with competitive predictive accuracy. This trade-off is consistent with Proposition~\ref{prop:tail_sparsity}: in sparse tail regimes, low-dimensional corrections achieve robust performance without incurring the variance penalties of more flexible alternatives.
