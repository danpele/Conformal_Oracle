...
\begin{document}
	...
	The full replication pipeline---from data download through TSFM inference, conformal calibration, backtesting, and figure generation---is available via the Quantlet \quantlet{https://github.com/QuantLet/Conformal_Oracle/} repository at \url{https://github.com/QuantLet/Conformal_Oracle}. The repository includes all Monte Carlo simulation code (Section~\ref{sec:simulation}) in addition to the empirical pipeline. All return data are sourced from Yahoo Finance (public). All foundation model checkpoints are open-weight and hosted on HuggingFace (Table~\ref{tab:models}); no proprietary API access is required.
