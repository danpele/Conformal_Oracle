...
\begin{document}
	...
	$\qV$ estimates.
	Disaggregating by asset class, the commodity Green rate increases from 64\% % TODO: confirm commodity Green% at w=250 from asset_class breakdown
	at $w=250$ to \texttt{[X]}\% at $w=500$, with WTI % TODO: fill in commodity w=500 numbers from robustness runs
	improving from 5/9 to \texttt{[Y]}/9 Green. The improvement confirms that extending
	the rolling window partially mitigates the score persistence issue identified
	in Section~\ref{sec:results_raw}; however, longer windows also increase
	the correction lag during regime transitions (Section~\ref{sec:discuss}).
	...
	The comparison extends to purpose-built VaR models. GARCH-N with conformal correction (Table~\ref{tab:master}, Panel~A: QS $= 4.7 \times 10^{-4}$, 21/24 Green) dominates both EVT-POT (QS $= 5.5$, 18/24 Green) and filtered historical simulation (QS $= 5.6$, 13/24 Green) under the short 250-day rolling windows typical of regulatory implementations, despite requiring no tail distribution fitting, no threshold selection, and no GPD estimation. Under longer estimation samples or carefully tuned thresholds, EVT-based methods may perform differently; the result should therefore be read as evidence of robustness under short-sample conditions rather than general dominance over parametric tail models. Table~\ref{tab:baselines} compares static recalibration methods head-to-head. Among static procedures, Adaptive Conformal Inference \citep{gibbs2021adaptive} achieves a higher Green rate than the static conformal shift at the cost of wider VaR intervals, placing static ACI and static conformal on a calibration--sharpness frontier. The rolling conformal variant, however, achieves a Green rate of 213/216 = 98.6\% together with competitive sharpness (Table~\ref{tab:rolling_vs_static}), resolving the apparent trade-off: the rolling correction dominates static ACI on the Green rate while retaining the sharpness advantage of the fixed-$\alpha$ conformal shift. This empirical ordering is consistent with Proposition~\ref{prop:tail_sparsity}: low-dimensional corrections achieve robust performance without incurring the variance penalties of more flexible alternatives, and temporal adaptation via a rolling window delivers further coverage gains without enlarging the parameter count.
	...
	\begin{table}[H]
		...
		\begin{tabular}{@{}lrrrrr@{}}
			...
			Conformal rolling $w{=}250$ (ours) & .009 & \texttt{[X]}/216 & \texttt{[X]} & \texttt{[X]} & 98.6 \\ % TODO: insert rolling pooled Kupiec rej., QS, Width
	...
	% TODO: Generate Figure I.1 with all 9 models on the frontier plot.
